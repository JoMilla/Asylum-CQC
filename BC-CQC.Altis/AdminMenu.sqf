...
Admin_List = compileFinal "['76561198151637619','_SP_PLAYER_']";		//replace these with your admin player UIDS (steamID64)
...
if(isServer) then {
	...
	AH_Menu_Para = {
		...
		_sender = _this select 1;
			...
			_target addBackpack "B_Parachute" call AH_fnc_MP;
			_target setPosATL (_target modelToWorld[0,0,1500]) call AH_fnc_MP;
			format["%1 has parachuted %2",name _sender,name _target] call SERVER_LOG;
			[{hint "The Target is up there!";},"BIS_fnc_Spawn",_sender,false] call AH_fnc_MP;
	...
	if(player call AH_AdminCheck) then {
		...
		if(isNil "AH_Init") then {
			...
			[] spawn {
				while{true} do {
					//if(!isNil "AH_BIND") then {(findDisplay 46) displayRemoveEventHandler["KeyDown",AH_BIND];};
					//AH_BIND = (findDisplay 46) displayAddEventHandler ["KeyDown",{if((_this select 1) == 61) then {[] call AH_Init;};if((_this select 1)
		...
		AH_ViewLogs = {
			...
			ctrlSetText[1000,"JoMilla's Server Log"];
		...
		AH_SpawnMenu = {
			...
			createDialog "RscDisplayChooseEditorLayout";
			disableSerialization;
			ctrlSetText[1000,"Spawn A Vehicle"];
			_ctrl = (findDisplay 164) displayctrl 1;
		...
		AH_AreYouSure = {
			...
			_responce = [_question, "The Admin Menu", "Yes", "No"] call BIS_fnc_guiMessage;
			_responce
		};
		AH_Init = {
			if(player call AH_AdminCheck) then {
				if(isNull (FindDisplay 163)) then {
					...
					_ctrl ctrlSetText "JoMilla Admin Menu";
					...
					_ctrl lbAdd "TP All Here";
					_ctrl lbAdd "Arsenal";
					_ctrl lbAdd "Parachute Player";
					_ctrl lbAdd "Storm";
					_ctrl lbAdd "Clear Weather";
					_ctrl lbAdd "Toggle Sun";
					_ctrl lbAdd "LaserTag";
					if(AH_LT) then {
						_ctrl lbSetColor [16,[0,1,0,1]];
					} else {
						_ctrl lbSetColor [16,[1,0,0,1]];
					};
		...
		AH_Target = {
			...
			disableserialization;
			createDialog "RscDisplayChooseEditorLayout";
			ctrlSetText[1000,"JoMilla Menu - Select a target"];
			if(_script == 0) then {
				_ctrl = (findDisplay 164) displayctrl 1100;
			...
			if(_script == 5) then {
				_ctrl = (findDisplay 164) displayctrl 1100;
				_ctrl ctrlSetStructuredText parseText "<t size='1.1'>Parachute</t><br/><t size='0.9'>Select a target to send them skyward!</t>";
				_ctrl ctrlCommit 0;

				_ctrl = (findDisplay 164) displayctrl 1;
				_ctrl ctrlSetText "Chute";
				_ctrl buttonSetAction "[[player,(lbText[101,lbCurSel 101] call AH_GetObject)],'AH_Menu_Para',false,false] call AH_fnc_MP";
				_ctrl ctrlCommit 0;
			};
			_ctrl = (findDisplay 164) displayctrl 101;
			{
		...
		AH_DBLClick = {
			...
			switch(_index) do {
				...
				case 9: {call AH_ESP;};
				case 10: {[] spawn AH_Menu_TPAHere; call BIS_fnc_MP;};
				case 11: {["Open",true] spawn BIS_fnc_arsenal;};
				case 12: {5 spawn AH_Target;};
				case 13: {["functions\storm.sqf","BIS_fnc_execVM",true,true ] call BIS_fnc_MP;};
				case 14: {["functions\stormoff.sqf","BIS_fnc_execVM",true,true ] call BIS_fnc_MP;};
				case 15: {skipTime 12 call BIS_fnc_MP;};
				case 16: {[] spawn AH_Lasertag call BIS_fnc_MP;};
			};
		};
		AH_CarGod = {
			if !(player call AH_AdminCheck) exitWith {};
		...
		AH_Lasertag = {
			...
			AH_LT = !AH_LT;
			if(AH_LT) then {
				["functions\laser.sqf","BIS_fnc_execVM",true,true ] call BIS_fnc_MP; 
				1 setFog 0.8  call BIS_fnc_MP; 
				PV_laserToggle = 1; 
				if (isServer) then {publicVariable "PV_laserToggle";};
				["functions\actions.sqf","BIS_fnc_execVM",true,true ] call BIS_fnc_MP;
				hint "LASERTAG: Keep your laser on!";
				
			} else {
				titleText ["Lasertag has ended", "BLACK IN", 10];
				PV_laserToggle = 0; 
				if (isServer) then {publicVariable "PV_laserToggle";};
				["functions\actions.sqf","BIS_fnc_execVM",true,true ] call BIS_fnc_MP;
				["functions\L1.sqf","BIS_fnc_execVM",true,true ] call BIS_fnc_MP;
			};
		};
		...
		hint parseText format["Press '%1' (Custom Control 20) to open the admin menu!<br/>Press '%2'(Custom Control 19) to open the spawn menu!<br/>F1 - Delete Vehicle<br/>F2 - Repair Vehicle/Player",(actionKeysNames ["User20",1]),(actionKeysNames ["User19",1])];
	};
};
